...
export async function fetchRenda(usuarioId, email, senha) {
    const res = await fetch(`${API_BASE}/renda/usuario/${usuarioId}`, {
        headers: { 'Authorization': basicAuthHeader(email, senha) },
        mode: 'cors'
    });
    if (!res.ok) {
        // Se for 404, pode ser que o usuário novo ainda não tenha a trigger disparada ou renda criada
        if (res.status === 404) return { rendaTotal: 0 };
        throw new Error('Falha ao buscar renda: ' + res.status);
    }
    return res.json();
}
...
export default {
    ...
    listGastosByUser, listGanhosByUser, fetchRenda
};
